...
def parse_bytes(clause_constructor, daide_bytes, on_error="raise"):
    ...
    assert on_error in (
        ...
        "warn",
        "ignore",
    ), 'Valid values for error are "raise", "warn", "ignore"'
...
class String(AbstractClause):
    ...
    def from_bytes(self, daide_bytes, on_error="raise"):
        ...
        if not str_group_bytes:
            self.error(
                on_error, "Unable to find a set of parentheses to extract the string clause."
            )
            return daide_bytes
...
class Province(AbstractClause):
    ...
    def from_bytes(self, daide_bytes, on_error="raise"):
        ...
        if province_group_bytes:
            ...
            province, province_group_bytes = parse_bytes(
                SingleToken, province_group_bytes, on_error=on_error
            )
            coast, province_group_bytes = parse_bytes(
                SingleToken, province_group_bytes, on_error=on_error
            )
            ...
            if province_group_bytes:
                self.error(
                    on_error,
                    "{} bytes remaining. Province is malformed".format(len(province_group_bytes)),
                )
                return daide_bytes
    ...
    def from_string(self, string, on_error="raise"):
        ...
        if province and coast:
            ...
            self._bytes = add_parentheses(
                bytes(Token(from_str=str_province)) + bytes(Token(from_str=str_coast))
            )
...
class Turn(AbstractClause):
    ...
    _alias_from_bytes = {"AUT": "F.R", "FAL": "F.M", "SPR": "S.M", "SUM": "S.R", "WIN": "W.A"}
    _alias_from_string = {value: key for key, value in _alias_from_bytes.items()}
    ...
    def from_bytes(self, daide_bytes, on_error="raise"):
        ...
        if turn_group_bytes:
            self.error(
                on_error, "{} bytes remaining. Turn is malformed".format(len(turn_group_bytes))
            )
            return daide_bytes
    ...
    def from_string(self, string, on_error="raise"):
        ...
        self._str = string
        self._bytes = add_parentheses(
            bytes(Token(from_str=str_season)) + bytes(Token(from_int=int(str_year)))
        )
...
class Unit(AbstractClause):
    ...
    def from_bytes(self, daide_bytes, on_error="raise"):
        ...
        if unit_group_bytes:
            self.error(
                on_error, "{} bytes remaining. Order is malformed".format(len(unit_group_bytes))
            )
            return daide_bytes
...
class Order(AbstractClause):
    ...
    def from_bytes(self, daide_bytes, on_error="raise"):
        ...
        if order_type_str == "WAIVE":
            ...
            # Move order
            if order_type_str == "-":
                province, order_group_bytes = parse_bytes(
                    Province, order_group_bytes, on_error=on_error
                )
                str_buffer += [str(province)]
                second_order_type, order_group_bytes = parse_bytes(
                    OrderType, order_group_bytes, on_error="ignore"
                )
            ...
            # Support
                ...
                other_unit, order_group_bytes = parse_bytes(
                    Unit, order_group_bytes, on_error=on_error
                )
            ...
            # Convoy
                ...
                second_order_type, order_group_bytes = parse_bytes(
                    OrderType, order_group_bytes, on_error=on_error
                )
                province, order_group_bytes = parse_bytes(
                    Province, order_group_bytes, on_error=on_error
                )
            ...
            self.error(
                on_error, "Unable to find a unit, a power or an order to build the order clause"
            )
            return daide_bytes

        if order_group_bytes:
            self.error(
                on_error, "{} bytes remaining. Order is malformed".format(len(order_group_bytes))
            )
            return daide_bytes
