...
def test_dict_type():
    ...
    checker = parsing.DictType(
        MyStringable, float, dict_builder=SortedDict.builder(MyStringable, float)
    )
...
def test_model_parsing():
    ...
    assert_raises(
        lambda: parsing.validate_data(bad_data_value, model), (exceptions.ValueException,)
    )
...
def test_indexed_sequence():
    ...
    checker = parsing.IndexedSequenceType(
        parsing.DictType(str, parsing.JsonableClassType(MyJsonable)), "field_b"
    )
    sequence = [
        MyJsonable(field_a=True, field_b="x1", field_e=[1, 2, 3], field_f=[1.0, 2.0, 3.0]),
        MyJsonable(field_a=True, field_b="x3", field_e=[1, 2, 3], field_f=[1.0, 2.0, 3.0]),
        MyJsonable(field_a=True, field_b="x2", field_e=[1, 2, 3], field_f=[1.0, 2.0, 3.0]),
        MyJsonable(field_a=True, field_b="x5", field_e=[1, 2, 3], field_f=[1.0, 2.0, 3.0]),
        MyJsonable(field_a=True, field_b="x4", field_e=[1, 2, 3], field_f=[1.0, 2.0, 3.0]),
    ]
