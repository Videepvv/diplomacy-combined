...
class Connection:
    ...
    def send(self, request, for_game=None):
        ...
        self.write_request(request_context).add_done_callback(
            _MessageWrittenCallback(request_context).callback
        )
...
class _Reconnection:
    ...
    def reconnect(self):
        ...
        for (
            waiting_context
        ) in self.connection.requests_waiting_responses.values():  # type: RequestFutureContext
    ...
    def callback(self, msg_future):
        ...
        if exception is not None:
            if isinstance(exception, (WebSocketClosedError, StreamClosedError)):
                ...
                LOGGER.error(
                    "Connection was closed when sending a request. Silently waiting for a reconnection."
                )
            else:
