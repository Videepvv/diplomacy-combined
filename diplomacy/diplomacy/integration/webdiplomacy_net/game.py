...
def unit_dict_to_str(unit_dict, map_id=1):
    ...
    if [1 for field in req_fields if field not in unit_dict]:
        LOGGER.error(
            "The required fields for unit dict are %s. Cannot translate %s", req_fields, unit_dict
        )
        return "", ""
...
def center_dict_to_str(center_dict, map_id=1):
    ...
    if [1 for field in req_fields if field not in center_dict]:
        LOGGER.error(
            "The required fields for center dict are %s. Cannot translate %s",
            req_fields,
            center_dict,
        )
        return "", ""
...
def order_dict_to_str(order_dict, phase, map_id=1):
    ...
    if [1 for field in req_fields if field not in order_dict]:
        LOGGER.error(
            "The required fields for order dict are %s. Cannot translate %s", req_fields, order_dict
        )
        return "", "", ""
...
def process_phase_dict(phase_dict, map_id=1):
    ...
    for order_dict in phase_dict.get("orders", []):
        power_name, order = order_dict_to_str(
            order_dict, phase=phase_dict.get("phase", "Diplomacy"), map_id=map_id
        )
        if not power_name:
            continue
...
def state_dict_to_game_and_power(state_dict, country_id, max_phases=None):
    ...
    if [1 for field in req_fields if field not in state_dict]:
        LOGGER.error(
            "The required fields for state dict are %s. Cannot translate %s", req_fields, state_dict
        )
        return None, None
    ...
    if current_phase["name"][-1] == "R":
        ...
        for power in game.powers.values():
            for retreat_unit in power.retreats:
                power.retreats[retreat_unit] = [
                    loc
                    for loc in power.retreats[retreat_unit]
