...
def _game_request_method(channel_method):
    ...
    """ % {
        ...
        "with_params": (
            " with forced parameters ``(%s)`` and additional request parameters "
            % (
...
class NetworkGame(Game):
    ...
    def __init__(self, channel, received_game):
        ...
        super(NetworkGame, self).__init__(
            **{key: getattr(received_game, key) for key in received_game.get_model()}
        )
    ...
    send_order_suggestions = _game_request_method(Channel._send_order_suggestions)
