...
class Renderer:
    ...
    def render(self, incl_orders=True, incl_abbrev=False, output_format="svg", output_path=None):
        ...
        nb_centers = [
            (power.name[:3], len(power.centers))
        ...
        nb_centers_per_power = " ".join(
            ["{}: {}".format(name, centers) for name, centers in nb_centers]
        )
            ...
            if incl_orders:
                ...
                for order_key in power.orders:
                    ...
                    if not tokens or len(tokens) < 3:
                        ...
                        if "-" in tokens:
                            src_loc = (
                                tokens[4] if tokens[3] == "A" or tokens[3] == "F" else tokens[3]
                            )
                ...
                if child_node.nodeName == "jdipNS:POWERCOLORS":
                    for power_color in child_node.childNodes:
                        if power_color.nodeName == "jdipNS:POWERCOLOR":
                            self.metadata["color"][_attr(power_color, "power").upper()] = _attr(
                                power_color, "color"
                            )
    ...
    def _set_influence(self, xml_map, loc, power_name, has_supply_center=False):
        ...
        if map_layer:
            for map_node in map_layer.childNodes:
                if map_node.nodeName in ("g", "path", "polygon") and map_node.getAttribute(
                    "id"
                ) == "_{}".format(loc.lower()):
    ...
    def _set_current_phase(xml_map, current_phase):
        ...
        """
        current_phase = (
            "FINAL" if current_phase[0] == "?" or current_phase == "COMPLETED" else current_phase
        )
        for child_node in xml_map.getElementsByTagName("svg")[0].childNodes:
    ...
    def _issue_move_order(self, xml_map, src_loc, dest_loc, power_name):
        ...
        vector_length = (delta_x**2 + delta_y**2) ** 0.5
        delta_dec = (
            float(self.metadata["symbol_size"][ARMY][1]) / 2 + 2 * self._colored_stroke_width()
        )
        dest_loc_x = str(
    ...
    def _issue_convoy_order(self, xml_map, loc, src_loc, dest_loc, power_name):
        ...
        dest_loc_x = str(
            round(
                src_loc_x + (dest_vector_length - delta_dec) / dest_vector_length * dest_delta_x, 2
            )
        )
        dest_loc_y = str(
            round(
                src_loc_y + (dest_vector_length - delta_dec) / dest_vector_length * dest_delta_y, 2
            )
        )
    ...
    def _get_unit_center(self, loc, is_dislodged):  # type: (str, bool) -> Tuple[float, float]
        ...
        unit_height, unit_width = self.metadata["symbol_size"][ARMY]
        return (float(unit_x) + float(unit_width) / 2, float(unit_y) + float(unit_height) / 2)
