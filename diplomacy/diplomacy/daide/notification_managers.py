...
def _build_completed_notifications(server_users, has_draw_vote, powers, state_history):
    ...
    daide_users = [
        daide_user if isinstance(daide_user, DaideUser) else None for daide_user in daide_users
    ]
    ...
    for phase, state in state_history.items():
        eliminated_powers = [
            ...
            if not powers_year_of_elimination[power_name]
            and (all(unit.startswith("*") for unit in units) or not units)
        ]
        for power_name in eliminated_powers:
            powers_year_of_elimination[power_name] = splitter.PhaseSplitter(phase.value).year

    years_of_elimination = [
        powers_year_of_elimination[power_name] for power_name in sorted(powers_year_of_elimination)
    ]
    notifs.append(
        notifications.SMR(last_phase.input_str, powers, daide_users, years_of_elimination)
    )
...
def on_processed_notification(server, notification, connection_handler, game):
    ...
    for order in previous_phase_data.orders[power_name]:
        ...
        notifs.append(
            notifications.ORD(
                previous_phase.input_str, order_bytes, [result.code for result in results]
            )
        )

    if game.status == strings.ACTIVE:
        notifs += _build_active_notifications(
            game.get_current_phase(), powers, game.map_name, game.deadline
        )

    elif game.status == strings.COMPLETED:
        notifs += _build_completed_notifications(
            server.users, game.has_draw_vote(), powers, game.state_history
        )
...
MAPPING = {
    diplomacy_notifications.GameProcessed: on_processed_notification,
