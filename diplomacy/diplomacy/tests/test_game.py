...
def test_histories():
    ...
    assert all(
        (p1 == p2 == p3)
        for (p1, p2, p3) in zip(
            game.message_history.keys(), game.order_history.keys(), game.state_history.keys()
        )
    )

    assert (
        game.map.compare_phases(
            str(game.state_history.first_key()), str(game.state_history.last_key())
        )
        < 0
    )
