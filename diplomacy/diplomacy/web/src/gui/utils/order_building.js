...
function assertLength(expected, given) {
    if (expected !== given) throw new Error(`Length error: expected ${expected}, given ${given}.`);
}

export class ProvinceCheck {
    static retreated(province, powerName) {
        const retreatProvince = province.getRetreated(powerName);
        if (!retreatProvince) throw new Error(`No retreated location at province ${province.name}.`);
    ...
    static present(province, powerName) {
        ...
        if (presenceProvince) {
            ...
            if (!presenceProvince) throw new Error(`No unit or retreat at province ${province.name}.`);
    ...
    static occupied(province, powerName) {
        ...
        if (!occupiedProvince) throw new Error(`No occupied location at province ${province.name}.`);
        // No confusion possible, we select the only occupied location at this province.
    ...
    static supportToString(path) {
        ...
        if (path[2].substr(2) !== path[3]) order += ` - ${path[3]}`;
        return order;
    }
