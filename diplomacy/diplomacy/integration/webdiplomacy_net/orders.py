...
def find_convoy_path(src, dest, map_object, game=None, including=None, excluding=None):
    ...
    water_locs = {
        loc.upper() for loc in map_object.locs if map_object.area_type(loc.upper()) == "WATER"
    }
    ...
    if game is not None:
        ...
        for power_units in game.get_units().values():
            convoyers |= {
                unit[2:] for unit in power_units if unit[0] == "F" and unit[2:] in water_locs
            }
    ...
    for nb_fleets in map_object.convoy_paths:
        for start_loc, fleet_locs, dest_locs in map_object.convoy_paths[nb_fleets]:
            ...
            if not fleet_locs.issubset(convoyers):  # Missing some convoyers to use this path
                continue
            if fleet_locs.intersection(including) != set(
                including
            ):  # Missing some fleets needed to be incl.
                continue
...
class Order:
    ...
    def _build_from_string(self, order, game=None):
        ...
        # {"id": "74", "unitID": "17", "type": "Move", "toTerrID": "69", "fromTerrID": "", "viaConvoy": "No"}
            ...
            if len(words) < 4:
                LOGGER.error(
                    '[Move] Unable to parse the move order "%s". Require at least 4 words', order
                )
                LOGGER.error(order)
                return
            ...
            if to_loc_name is None:
                LOGGER.error(
                    '[Move] Received invalid to loc "%s" for map "%s".', to_terr_id, self.map_name
                )
                LOGGER.error(order)
                return
            ...
            if convoy_path:
                self.order_dict["convoyPath"] = [
                    CACHE[self.map_name]["loc_to_ix"][loc] for loc in convoy_path[:-1]
                ]
        ...
        # {"id": "73", "unitID": "16", "type": "Support hold", "toTerrID": "24", "fromTerrID": "", "viaConvoy": ""}
            ...
            if len(words) < 5:
                LOGGER.error(
                    '[Support H] Unable to parse the support hold order "%s". Require at least 5 words',
                    order,
                )
                LOGGER.error(order)
                return
            ...
            if to_loc_name is None:
                LOGGER.error(
                    '[Support H] Received invalid to loc "%s" for map "%s".',
                    to_terr_id,
                    self.map_name,
                )
                LOGGER.error(order)
        ...
        # {"id": "73", "unitID": "16", "type": "Support move", "toTerrID": "24", "fromTerrID": "69", "viaConvoy": ""}
            ...
            if len(words) < 6:
                LOGGER.error(
                    'Unable to parse the support move order "%s". Require at least 6 words', order
                )
                return
            ...
            if to_loc_name is None:
                LOGGER.error(
                    '[Support M] Received invalid to loc "%s" for map "%s".',
                    to_terr_id,
                    self.map_name,
                )
                LOGGER.error(order)
            ...
            if from_loc_name is None:
                LOGGER.error(
                    '[Support M] Received invalid from loc "%s" for map "%s".',
                    from_terr_id,
                    self.map_name,
                )
                LOGGER.error(order)
            ...
            if words[move_index - 2] != "F" and map_object.area_type(from_loc_name) == "COAST":
                convoy_path = find_convoy_path(
                    from_loc_name, to_loc_name, map_object, game, excluding=loc_name
                )

            self.order_str = "%s %s S %s - %s" % (
                short_unit_type,
                loc_name,
                from_loc_name,
                to_loc_name,
        ...
        # "convoyPath": ["20", "69"]}
            ...
            if len(words) < 6:
                LOGGER.error(
                    '[Convoy] Unable to parse the convoy order "%s". Require at least 6 words',
                    order,
                )
                LOGGER.error(order)
                return
            ...
            if to_loc_name is None:
                LOGGER.error(
                    '[Convoy] Received invalid to loc "%s" for map "%s".', to_terr_id, self.map_name
                )
                LOGGER.error(order)
                return
            if from_loc_name is None:
                LOGGER.error(
                    '[Convoy] Received invalid from loc "%s" for map "%s".',
                    from_terr_id,
                    self.map_name,
                )
                LOGGER.error(order)
            ...
            convoy_path = find_convoy_path(
                from_loc_name, to_loc_name, map_object, game, including=loc_name
            )

            self.order_str = "%s %s C A %s - %s" % (
                short_unit_type,
                loc_name,
                from_loc_name,
                to_loc_name,
        ...
        # {"id": "152", "unitID": "18", "type": "Retreat", "toTerrID": "75", "fromTerrID": "", "viaConvoy": ""}
            ...
            if len(words) < 4:
                LOGGER.error(
                    '[Retreat] Unable to parse the move order "%s". Require at least 4 words', order
                )
                LOGGER.error(order)
                return
    ...
    def _build_from_dict(self, order):
        ...
        if via_convoy not in ("Yes", "No", "", None):
            LOGGER.error(
                'Received invalid via convoy "%s". Expected "Yes" or "No" or "".', via_convoy
            )
            return
