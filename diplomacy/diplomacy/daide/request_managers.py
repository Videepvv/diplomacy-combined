...
def on_name_request(server, request, connection_handler, game):
    ...
    power_name = [
        power_name for power_name, power in game.powers.items() if not power.is_controlled()
    ]
    if not power_name:
        return [responses.REJ(bytes(request))]
...
def on_history_request(server, request, connection_handler, game):
    ...
    for order in phase_order[power_name]:
        ...
        history_responses.append(
            notifications.ORD(phase.input_str, order_bytes, [result.code for result in results])
        )
...
def on_submit_orders_request(server, request, connection_handler, game):
    ...
    while request_bytes:
        ...
        new_power_adjusts = [
            adjust for adjust in power.adjust if adjust not in initial_power_adjusts
        ]
        new_power_orders = {
            id: val for id, val in power.orders.items() if id not in initial_power_orders
        }
...
def on_send_message_request(server, request, connection_handler, game):
    ...
    message = " ".join(
        [
            str(tokens.Token(from_bytes=(request.message_bytes[i], request.message_bytes[i + 1])))
...
def on_accept_request(server, request, connection_handler, game):
    ...
    if bytes(lead_token) == bytes(tokens.MAP):
        ...
        if not power_name:
            uncontrolled_powers = [
                pow_name
                for pow_name, power in game.powers.items()
            ...
            if ":" in daide_user.client_name:
                ...
                selected_powers = [
                    pow_name
                    for pow_name in uncontrolled_powers
