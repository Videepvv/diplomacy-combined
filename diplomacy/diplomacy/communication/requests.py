...
class JoinGame(_AbstractChannelRequest):
    ...
    params = {
        ...
        strings.POWER_NAME: parsing.OptionalValueType(str),
        strings.REGISTRATION_PASSWORD: parsing.OptionalValueType(str),
        strings.PLAYER_TYPE: parsing.OptionalValueType(
            parsing.EnumerationType(strings.ALL_PLAYER_TYPES)
        ),
    }
...
class SendOrderSuggestions(_AbstractGameRequest):
    ...
    params = {"suggestions": parsing.SequenceType(str), "power": parsing.OptionalValueType(str)}

    def __init__(self, **kwargs):
        self.suggestions = None
        self.power = None
        super(SendOrderSuggestions, self).__init__(**kwargs)
