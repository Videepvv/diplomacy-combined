...
export class Channel {
    ...
    _req(name, forcedParameters, localChannelFunction, parameters, game) {
        ...
        if (level === STRINGS.GAME) {
            ...
            if (game) throw new Error("A game object should not be provided for a non-game request.");
        ...
        const timeoutID = setTimeout(function () {
            if (!future.done())
                future.setException(
                    "Timeout reached when trying to send a request " + name + "/" + request.request_id + ".",
                );
