...
def test_drop():
    ...
    assert not [
        loc_name for (loc_name, loc) in list(this_map.loc_name.items()) if loc.startswith("STP")
    ]
    assert not [
        alias for (alias, value) in list(this_map.aliases.items()) if value.startswith("STP")
    ]
    assert not [homes for homes in list(this_map.homes.values()) if "STP" in homes]
    assert not [
        units for units in list(this_map.units.values()) for unit in units if unit[2:5] == "STP"[:3]
    ]
    assert not [center for center in list(this_map.scs) if center.upper().startswith("STP")]
    assert not [
        p_name
        for (p_name, scs) in this_map.centers.items()
        for center in scs
        if center.startswith("STP")
    ]
    assert not [
        loc
        for loc, abuts in list(this_map.loc_abut.items())
        for there in abuts
...
def test_compact():
    ...
    assert this_map.compact(
        "England: Fleet Western Mediterranean -> Tyrrhenian Sea. (*bounce*)"
    ) == ["F", "WES", "TYS", "|"]
...
def test_norm():
    ...
    this_map = deepcopy(Map())
    assert (
        this_map.norm("abc def. ghi/jkl!-ABC|~ (Hello)") == "ABC DEF GHI /JKL ! ABC | ~ ( HELLO )"
    )
...
def test_find_next_phase():
    ...
    assert (
        this_map.find_next_phase("SPRING 1902 RETREATS", phase_type="M", skip=1)
...
def test_find_previous_phase():
    ...
    assert (
        this_map.find_previous_phase("SPRING 1902 MOVEMENT", phase_type="R") == "FALL 1901 RETREATS"
    )
    assert (
        this_map.find_previous_phase("SPRING 1903 MOVEMENT", phase_type="R", skip=1)
