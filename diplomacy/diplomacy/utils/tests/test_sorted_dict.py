...
def test_builder_and_properties():
    ...
    assert (
        isinstance(sorted_dict, SortedDict)
        and sorted_dict.key_type is float
        and sorted_dict.val_type is bool
    )
...
def test_items_functions():
    ...
    # Test setter.
    sorted_dict["panthera"] = -0.88
    sorted_dict["cat"] = 2223.0
    # Test put().
...
def test_item_deletion_and_remove():
    ...
    sorted_dict = SortedDict(
        str, float, {"lion": 1.5, "tiger": -2.7, "panthera": -0.88, "cat": 2223.0, "serval": 39e12}
    )
...
def test_iterations():
    ...
    expected_sorted_values = [2223.0, 1.5, -0.88, 39e12, -2.7]
    sorted_dict = SortedDict(
        str, float, {"lion": 1.5, "tiger": -2.7, "panthera": -0.88, "cat": 2223.0, "serval": 39e12}
    )
...
def test_sub_and_remove_sub():
    ...
    sorted_dict = SortedDict(
        int, str, {k: "value of %s" % k for k in (2, 5, 1, 9, 4, 5, 20, 0, 6, 17, 8, 3, 7, 0, 4)}
    )
