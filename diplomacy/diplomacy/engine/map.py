...
class Map:
            ...
            for line in file:
                ...
                # -- [oldAbbrev ->] placeName = abbreviation alias...
                    ...
                    for alias in word[1:]:
                        ...
                        normed = (
                            alias[:-1].replace("+", " ").upper() if unclear else self.norm(alias)
                        )
                        if unclear:
                ...
                # -- DUMMIES [ALL EXCEPT] powerName...
                    ...
                    # DUMMY powerName powerName
                        ...
                        self.dummies.extend(
                            [
                                dummy
                                for dummy in [self.norm_power(p_name) for p_name in word[1:]]
                    ...
                    # DUMMY ALL EXCEPT powerName powerName
                        ...
                        self.dummies = [
                            ...
                            not in (
                                ["UNOWNED"]
                                + [self.norm_power(except_pow) for except_pow in word[3:]]
                            )
                        ]
                ...
                # -- UNPLAYED [ALL EXCEPT] powerName...
                    ...
                    # UNPLAYED ALL EXCEPT powerName
                        ...
                        goners = [
                            ...
                            not in (
                                ["UNOWNED"]
                                + [self.norm_power(pow_except) for pow_except in word[3:]]
                            )
                        ]
    ...
    def abuts(self, unit_type, unit_loc, order_type, other_loc):
        ...
        **Note**: This method uses the precomputed cache
        ...
        if unit_type == "?":
            ...
            ) or self.abuts_cache.get(("F", unit_loc.upper(), order_type, other_loc.upper()), 0)
    ...
    def compare_phases(self, phase1, phase2):
        ...
        if phase1[-1] == "?":
            phase1 = (
                phase1[:-1]
                + [season.split()[1][0] for season in self.seq if season[0] == phase1[0]][-1]
            )
        if phase2[-1] == "?":
            phase2 = (
                phase2[:-1]
                + [season.split()[1][0] for season in self.seq if season[0] == phase2[0]][-1]
            )
        ...
        if len(now1) < 3 or len(now2) < 3:
            order1 = (
                len(now1) > 2 and 2 or phase1 == "FORMING" and 1 or phase1 == "COMPLETED" and 3 or 0
            )
            order2 = (
                len(now2) > 2 and 2 or phase2 == "FORMING" and 1 or phase2 == "COMPLETED" and 3 or 0
            )
        ...
        if season_ix1 > season_ix2:
            return (
                -1
                if "NEWYEAR" in [x.split()[0] for x in self.seq[(season_ix2) + (1) : season_ix1]]
                else 1
            )
        if season_ix1 < season_ix2:
            return (
                1
                if "NEWYEAR" in [x.split()[0] for x in self.seq[(season_ix1) + (1) : season_ix2]]
                else -1
            )
        return 0
