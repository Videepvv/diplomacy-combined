...
class RequestBuilder:
    ...
    def from_bytes(daide_bytes, **kwargs):
        ...
        if initial_bytes not in __REQUEST_CONSTRUCTORS__:
            raise ValueError(
                "Unable to find a constructor for %s" % str(Token(from_bytes=initial_bytes))
            )
...
class DaideRequest(_AbstractGameRequest):
    ...
    def __init__(self, **kwargs):
        ...
        self._bytes = b""
        self._str = ""
        super(DaideRequest, self).__init__(token="", game_id="", game_role="", phase="", **kwargs)
    ...
    def parse_bytes(self, daide_bytes):
        ...
        assert (
            len(daide_bytes) % 2 == 0
        ), "Expected request to have an even number of bytes. Got %d" % len(daide_bytes)
        ...
        while daide_bytes:
            ...
            pad = (
                ...
                if (
                    ...
                    or self._str[-1] == "("
                    or new_str == ")"
                    or (is_ascii_token(token) and new_str != "(")
                )
                else " "
            )
...
class NameRequest(DaideRequest):
    ...
    params = {strings.CLIENT_NAME: str, strings.CLIENT_VERSION: str}
...
class IAmRequest(DaideRequest):
    ...
    params = {strings.POWER_NAME: str, strings.PASSCODE: parsing.OptionalValueType(int)}
    ...
    def parse_bytes(self, daide_bytes):
        ...
        assert (
            not power_group_bytes
        ), "%s bytes remaining in power group. Request is malformed" % len(power_group_bytes)
...
class SubmitOrdersRequest(DaideRequest):
    ...
    def parse_bytes(self, daide_bytes):
        ...
        self.power_name = (
            None if not orders or not orders[0].power_name else str(orders[0].power_name)
        )
...
class TimeToDeadlineRequest(DaideRequest):
    ...
    __slots__ = ["seconds"]
    params = {strings.SECONDS: parsing.OptionalValueType(int)}
...
class DrawRequest(DaideRequest):
    ...
    __slots__ = ["powers"]
    params = {strings.POWERS: parsing.SequenceType(str)}
...
class NotRequest(DaideRequest):
    ...
    __slots__ = ["request"]
    params = {strings.REQUEST: parsing.JsonableClassType(DaideRequest)}
...
class AcceptRequest(DaideRequest):
    ...
    params = {strings.RESPONSE_BYTES: bytes}
...
class ParenthesisErrorRequest(DaideRequest):
    ...
    params = {strings.MESSAGE_BYTES: bytes}
...
class AdminMessageRequest(DaideRequest):
    ...
    params = {strings.ADM_MESSAGE: str}
...
# Constants
__REQUEST_CONSTRUCTORS__ = {
    bytes(tokens.NME): NME,
