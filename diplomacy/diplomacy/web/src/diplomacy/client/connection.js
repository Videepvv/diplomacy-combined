...
class ConnectionProcessing {
    ...
    stop() {
        if (!this.isConnected) {
            if (this.connection.socket) this.connection.socket.onopen = null;
            if (this.timeoutID) {
                clearTimeout(this.timeoutID);
                this.timeoutID = null;
            }
...
export class Connection {
    constructor(hostname, port, useSSL) {
        if (useSSL) Diplog.info(`Using SSL.`);
    ...
    onSocketClose(closeEvent) {
        if (this.closed) Diplog.info("Disconnected.");
        else {
    ...
    __write_request(requestContext) {
        ...
        if (request.name === STRINGS.SYNCHRONIZE) this.isConnecting.wait().then(onConnected, onAnyError);
        else this.isReconnecting.wait().then(onConnected, onAnyError);
