...
class API(BaseAPI):
    ...
    def list_games_with_players_in_cd(self):
        ...
        if response.code == 200 and response.body:
            try:
                ...
                LOGGER.warning(
                    'ERROR during "%s". Unable to load JSON: %s.',
                    route,
            ...
            for game_player in list_games_players:
                return_val += [
                    GameIdCountryId(
                        game_id=game_player["gameID"], country_id=game_player["countryID"]
                    )
                ]
        ...
        # Error Occurred
            ...
            LOGGER.warning(
                'ERROR during "%s". Error code: %d. Body: %s.', route, response.code, response.body
            )
    ...
    def get_game_and_power(self, game_id, country_id, max_phases=None):
        ...
        route = "game/status"
        url = "%s?%s" % (
            API_WEBDIPLOMACY_NET,
            urlencode({"route": route, "gameID": game_id, "countryID": country_id}),
        )
        ...
        if response.code == 200 and response.body:
            try:
                ...
                return return_val
            game, power_name = state_dict_to_game_and_power(
                state_dict, country_id, max_phases=max_phases
            )
    ...
    def set_orders(self, game, power_name, orders, wait=None):
        ...
        orders_dict = [
            Order(order, map_name=game.map_name, phase_type=game.phase_type, game=game)
            for order in orders
        ]
        ...
        if current_phase != "COMPLETED":
            season, current_year, phase_type = (
                current_phase[0],
        ...
        if not response.body:
            LOGGER.warning(
                'WARNING during "%s". No response body received. Is the server OK?', route
            )
            return False
        ...
        try:
            ...
            LOGGER.warning(
                'ERROR during "%s". Unable to load JSON: %s.', route, response.body.decode("utf-8")
            )
            return False
        orders_dict = [
            Order(order, map_name=game.map_name, phase_type=game.phase_type)
            for order in response_body
        ]
        all_orders_set = True
        ...
        for unit in submitted_orders:
            if submitted_orders[unit] != received_orders.get(unit, ""):
                ...
                LOGGER.warning(
                    '[%s/%s/%s]. Submitted: "%s" - Server has: "%s".',
                    game.game_id,
                    game.get_current_phase(),
                    power_name,
                    submitted_orders[unit],
                    received_orders.get(unit, ""),
                )
