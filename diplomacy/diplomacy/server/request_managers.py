...
def on_create_game(server, request, connection_handler):
    ...
    game_id, token, power_name, state, daide_port = (
        request.game_id,
        request.token,
        request.power_name,
        request.state,
...
def on_get_dummy_waiting_powers(server, request, connection_handler):
    ...
    return responses.DataGamesToPowerNames(
        data=server.get_dummy_waiting_power_names(request.buffer_size, request.token),
        request_id=request.request_id,
    )
...
def on_join_game(server, request, connection_handler):
    ...
    # Power name given, request sender wants to be a player.
        ...
        # Power not already controlled by request sender.
            ...
            Notifier(server, ignore_addresses=[(power_name, token)]).notify_game_powers_controllers(
                server_game
            )
...
def on_leave_game(server, request, connection_handler):
    ...
    if level.is_power():
        ...
        Notifier(server, ignore_addresses=[request.address_in_game]).notify_game_powers_controllers(
            level.game
        )
...
def on_list_games(server, request, connection_handler):
    ...
    for game_id in server.get_game_indices():
        if request.game_id and not (
            game_id.lower() in request.game_id.lower() or request.game_id.lower() in game_id.lower()
        ):
            continue
        server_game = server.load_game(game_id)
...
def on_send_game_message(server, request, connection_handler):
    ...
    """
    level = verify_request(
        server, request, connection_handler, omniscient_role=True, observer_role=False
    )
        ...
        if to_save:
            ...
            for server_game in server.games.values():  # type: ServerGame
                ...
                if not server_game.is_moderator(username) and not server_game.is_omniscient(
                    ...
                    transfer_special_tokens(
                        server_game, server, username, grade_update, grade_update == strings.PROMOTE
                    )
...
def on_send_log_data(server, request, connection_handler):
    ...
    level = verify_request(
        server, request, connection_handler, omniscient_role=True, observer_role=True
    )
    assert_game_not_finished(level.game)
