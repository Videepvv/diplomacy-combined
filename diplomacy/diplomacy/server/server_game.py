...
class ServerGame(Game):
    ...
    model = parsing.update_model(
        Game.model,
        {
            strings.MODERATOR_USERNAMES: parsing.DefaultValueType(
            ...
            strings.OMNISCIENT_USERNAMES: parsing.DefaultValueType(
                parsing.SequenceType(str, sequence_builder=set), ()
            ),
        },
    )
    ...
    def new_system_message(self, recipient, body):
        ...
        :rtype: Message
        """
        assert recipient in {GLOBAL, OBSERVER, OMNISCIENT} or self.has_power(recipient)
        message = Message(
            phase=self.current_short_phase, sender=SYSTEM, recipient=recipient, message=body
        )
    ...
    def has_token(self, token):
        ...
        return (
            self.omniscient.has_token(token)
            or self.observer.has_token(token)
            or any(power.has_token(token) for power in self.powers.values())
        )
    ...
    def filter_usernames(self, filter_function):
        ...
        self.omniscient_usernames = set(
            username for username in self.omniscient_usernames if filter_function(username)
        )
        self.moderator_usernames = set(
            username for username in self.moderator_usernames if filter_function(username)
        )
        for power in self.powers.values():
    ...
    def filter_tokens(self, filter_function):
        ...
        self.observer.remove_tokens(
            [token for token in self.observer.tokens if not filter_function(token)]
        )
        self.omniscient.remove_tokens(
            [token for token in self.omniscient.tokens if not filter_function(token)]
        )
        for power in self.powers.values():  # type: Power
