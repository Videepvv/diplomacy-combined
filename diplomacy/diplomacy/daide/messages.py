...
class DaideMessage(metaclass=ABCMeta):
    ...
    def from_stream(stream):
        ...
        # Creating message
        message_cls = {
            MessageType.INITIAL.value: InitialMessage,
...
class InitialMessage(DaideMessage):
    ...
    def __bytes__(self):
        ...
        return bytes(
            [
                MessageType.INITIAL.value,  # Message Type
                0,  # Padding
                0,
                4,  # Remaining length (2 bytes)
                0,
                DAIDE_VERSION,  # Daide version (2 bytes)
                0xDA,
                0x10,
            ]
    ...
    def build(self, stream, remaining_length):
        ...
        if remaining_length != 4:
            LOGGER.error(
                "Expected 4 bytes remaining in initial message. Got %d. Aborting.", remaining_length
            )
            self.is_valid = False
            return
...
class DiplomacyMessage(DaideMessage):
    ...
    def __bytes__(self):
        ...
        header = bytes(
            [
                MessageType.DIPLOMACY.value,  # Message Type
                0,  # Padding
                len(self.content) // 256,
                len(self.content) % 256,
            ]
        )  # Remaining length
...
class ErrorMessage(DaideMessage):
    ...
    def __bytes__(self):
        ...
        return bytes(
            [
                MessageType.ERROR.value,  # Message Type
                0,  # Padding
                0,
                2,  # Remaining length (2 bytes)
                0,
                error_code,
            ]
        )  # Error code (2 bytes)
