...
class Token:
    ...
    def _load_from_bytes(self, from_bytes):
        ...
        if len(from_bytes) != 2:
            raise ValueError(
                "Expected a couple of 2 bytes 0x000xFF. Got [{}]".format(
                    "".join([hex(b) for b in from_bytes])
                )
            )
    ...
    def __eq__(self, other):
        ...
        return isinstance(other, Token) and (
            (self.repr_int and self.repr_int == other.repr_int) or self.repr_str == other.repr_str
        )
...
def is_ascii_token(token):
    ...
    return (
        ...
        and len(token.repr_bytes) == 2
        and token.repr_bytes[0] == ASCII_BYTE
    )
...
def is_integer_token(token):
    ...
    """
    return isinstance(token, Token) and len(token.repr_bytes) == 2 and token.repr_bytes[0] < 64
