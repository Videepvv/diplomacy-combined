...
class Power(Jsonable):
    ...
    model = {
        ...
        strings.CONTROLLER: parsing.DefaultValueType(
        ...
        strings.VOTE: parsing.DefaultValueType(
            parsing.EnumerationType(strings.ALL_VOTE_DECISIONS), strings.NEUTRAL
        ),
        strings.WAIT: parsing.DefaultValueType(bool, True),
        strings.COMM_STATUS: parsing.DefaultValueType(str, strings.READY),
        strings.PLAYER_TYPE: parsing.DefaultValueType(
            parsing.EnumerationType(strings.ALL_PLAYER_TYPES), strings.NONE
        ),
    }
    ...
    def __str__(self):
        ...
        if show_retreats:
            text += "\n".join(
                [""] + [" ".join([unit, "-->"] + places) for unit, places in self.retreats.items()]
            )
    ...
    def reinit(self, include_flags=6):
        ...
        if reinit_transient:
            ...
            for dis_unit in self.retreats:
                self.game.update_hash(
                    self.name, unit_type=dis_unit[0], loc=dis_unit[2:], is_dislodged=True
                )
            self.homes = None
    ...
    def merge(self, other_power):
        ...
        for unit in list(other_power.retreats.keys()):
            ...
            self.game.update_hash(
                other_power.name, unit_type=unit[0], loc=unit[2:], is_dislodged=True
            )
