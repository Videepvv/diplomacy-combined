...
export class Game {
    ...
    assertPlayerGame(powerName) {
        if (!this.isPlayerGame(powerName))
            throw new Error(
                "Expected a player game" + (powerName ? " " + powerName : "") + ", got role " + this.role + ".",
            );
    }

    assertObserverGame() {
        if (!this.isObserverGame()) throw new Error("Expected an observer game, got role " + this.role + ".");
    }

    assertOmniscientGame() {
        if (!this.isOmniscientGame()) throw new Error("Expected an omniscient game, got role " + this.role + ".");
    }

    clearCenters(powerName) {
        for (let power_name of Object.keys(this.powers)) {
            if (!powerName || power_name === powerName) this.powers[power_name].clearCenters();
        }
    }

    clearOrders(powerName) {
        for (let power_name of Object.keys(this.powers))
            if (!powerName || power_name === powerName) this.powers[power_name].clearOrders();
    }

    clearUnits(powerName) {
        for (let power_name of Object.keys(this.powers)) {
            if (!powerName || power_name === powerName) this.powers[power_name].clearUnits();
        }
    }

    clearVote() {
        for (let power_name of Object.keys(this.powers)) this.powers[power_name].vote = "neutral";
    }

    countControlledPowers() {
        let count = 0;
        for (let power of Object.values(this.powers)) count += power.isControlled() ? 1 : 0;
        return count;
    }
    ...
    getPhaseType() {
        if (this.phase === null || this.phase === "FORMING" || this.phase === "COMPLETED") return null;
        return this.phase[this.phase.length - 1];
    }

    getControllablePowers() {
        if (this.isObserverGame() || this.isOmniscientGame()) return Object.keys(this.powers);
        return [this.role];
    }
    ...
    _build_sites(power) {
        ...
        const buildSites = [];
        for (let h of homes) {
            if (power.centers.includes(h) && !occupiedLocations.includes(h)) buildSites.push(h);
        }
        return buildSites;
    }
