...
class ClientCommsSimulator:
    ...
    def pop_next_request(self, comms):
        ...
        while com and com.client_id == self._id:
            if com.request:
                ...
                LOGGER.info(
                    "[%d:%d] preparing to send request [%s]",
                    self._id,
                    self.stream.socket.fileno() + 1,
                    request,
                )
                break
    ...
    def validate_resp_notifs(self, expected_resp_notifs):
        ...
        while expected_resp_notifs:
            ...
            LOGGER.info(
                "[%d:%d] Received reply [%s]",
                self._id,
                self.stream.socket.fileno() + 1,
                str(resp_notif),
            )
            LOGGER.info(
                "[%d:%d] Replies in buffer [%s]",
                self._id,
                self.stream.socket.fileno() + 1,
    ...
    def execute(self):
        ...
        try:
            ...
            while self._comms and (
                ...
                if (
                    next_comm.client_id not in self._clients
...
def test_game_reject_map():
    ...
    run_with_timeout(
        lambda: run_game_data(1, ["NO_PRESS", "IGNORE_ERRORS", "POWER_CHOICE"], game_path), 60
    )
...
def test_game_7():
    ...
    run_with_timeout(
        lambda: run_game_data(7, ["NO_PRESS", "IGNORE_ERRORS", "POWER_CHOICE"], game_path), 60
    )
