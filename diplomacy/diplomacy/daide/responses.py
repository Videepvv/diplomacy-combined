...
class MapNameResponse(DaideResponse):
    ...
    def __init__(self, map_name, **kwargs):
        ...
        self._bytes = bytes(tokens.MAP) + bytes(parse_string(String, map_name))
...
class MapDefinitionResponse(DaideResponse):
    ...
    def __init__(self, map_name, **kwargs):
        ...
        self._bytes = bytes(tokens.MDF) + powers_clause + provinces_clause + adjacencies_clause
    ...
    def _build_provinces_clause(game_map):
        ...
        power_names_centers = [
            (power_name, centers[:]) for power_name, centers in game_map.centers.items()
        ]
    ...
    def _build_adjacencies_clause(game_map):
        ...
        for province in sorted([loc.upper() for loc in game_map.locs if "/" not in loc]):
            ...
            adjacencies[province].setdefault(
                "A", []
            )  # List of adjacent provinces where armies can move
            ...
            for coast in ("A", "", "/EC", "/NC", "/SC", "/WC"):
                ...
                # (Fleet coast provinces): ((FLT coast) adjacent_prov adjacent_prov ...)
                    ...
                    coast_flt_adjacencies_clause = [
                        add_parentheses(flt_clause + coast_clause)
...
class SupplyCenterResponse(DaideResponse):
    ...
    def __init__(self, powers_centers, map_name, **kwargs):
        ...
        self._bytes = bytes(tokens.SCO) + b"".join(
            [add_parentheses(power_bytes) for power_bytes in all_powers_bytes]
        )
    ...
    def __init__(self, phase_name, powers_units, powers_retreats, **kwargs):
        ...
        for power_name, units in sorted(powers_units.items()):
            ...
            for unit, retreat_provinces in sorted(powers_retreats[power_name].items()):
                unit_clause = parse_string(Unit, "%s %s" % (power_name, unit))
                retreat_clauses = [
                    parse_string(Province, province) for province in retreat_provinces
                ]
                units_bytes_buffer += [
                    add_parentheses(
                        ...
                        + bytes(tokens.MRT)
                        + add_parentheses(
                            b"".join([bytes(province) for province in retreat_clauses])
                        )
                    )
                ]
...
class MissingOrdersResponse(DaideResponse):
    ...
    def _build_movement_phase(self, power):
        ...
        # Storing full response
        self._bytes = bytes(tokens.MIS) + b"".join(
            [
                bytes(parse_string(Unit, "%s %s" % (power.name, unit)))
    ...
    def _build_retreat_phase(self, power):
        ...
        units_with_no_order = {
            unit: retreat_provinces for unit, retreat_provinces in power.retreats.items()
        }
        ...
        for unit, retreat_provinces in sorted(
            units_with_no_order.items(), key=lambda key_val: key_val[0].split()[-1]
        ):
            ...
            units_bytes_buffer += [
                add_parentheses(
                    strip_parentheses(bytes(unit_clause))
                    + bytes(tokens.MRT)
                    + add_parentheses(b"".join([bytes(province) for province in retreat_clauses]))
                )
            ]
...
class SyntaxErrorResponse(DaideResponse):
    ...
    def __init__(self, request_bytes, error_index, **kwargs):
        ...
        message_with_err = (
            request_bytes[:error_index] + bytes(tokens.ERR) + request_bytes[error_index:]
        )
