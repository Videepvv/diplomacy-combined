...
def test_hlo():
    ...
    daide_str = (
        "HLO ( FRA ) ( #1234 ) ( ( LVL #0 ) ( MTL #1200 ) ( RTL #1200 ) ( BTL #1200 ) ( AOA ) )"
    )
...
def test_now():
    ...
    daide_str = (
        "NOW ( SPR #1901 ) ( AUS AMY BUD ) ( AUS AMY VIE ) ( AUS FLT TRI ) ( ENG FLT EDI )"
...
def test_ord_001():
    ...
    response = responses.ORD(
        phase_name=phase_name, order_bytes=str_to_bytes(order_daide_str), results=[]
    )
...
def test_ord_002():
    ...
    phase_name = game.map.phase_abbr(game.phase)
    response = responses.ORD(
        phase_name=phase_name, order_bytes=str_to_bytes(order_daide_str), results=[BOUNCE.code]
    )
    assert isinstance(response, responses.ORD), "Expected a ORD response"
    assert bytes(response) == str_to_bytes(daide_str)
...
def test_ord_003():
    ...
    phase_name = game.map.phase_abbr(game.phase)
    response = responses.ORD(
        phase_name=phase_name, order_bytes=str_to_bytes(order_daide_str), results=[DISLODGED]
    )
    assert isinstance(response, responses.ORD), "Expected a ORD response"
    assert bytes(response) == str_to_bytes(daide_str)
