...
def from_saved_game_format(saved_game):
    ...
    kwargs = {
        strings.MAP_NAME: saved_game.get("map", "standard"),
...
def load_saved_games_from_disk(input_path, on_error="raise"):
    ...
    assert on_error in (
        ...
        "warn",
        "ignore",
    ), 'Expected values for on_error are "raise", "warn", "ignore".'
...
def is_valid_saved_game(saved_game):
    ...
    for phase_ix in range(nb_phases):
        ...
        previous_phase_name = (
            "FORMING" if phase_ix == 0 else saved_game["phases"][phase_ix - 1]["name"]
        )
        next_phase_name = (
            "COMPLETED" if phase_ix == nb_phases - 1 else saved_game["phases"][phase_ix + 1]["name"]
        )
        ...
        if phase_ix < (nb_phases - 1):
            ...
            game = Game(
                saved_game["id"],
                map_name=saved_game["map"],
                rules=["SOLITAIRE"] + saved_game["rules"],
            )
