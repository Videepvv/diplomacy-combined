...
export class SupportHold extends React.Component {
    render() {
        ...
        dest_loc_x =
            "" + Math.round((parseFloat(loc_x) + ((vector_length - delta_dec) / vector_length) * delta_x) * 100) / 100;
        dest_loc_y =
            "" + Math.round((parseFloat(loc_y) + ((vector_length - delta_dec) / vector_length) * delta_y) * 100) / 100;

        return (
            <g stroke={Colors[this.props.powerName]} opacity={opacity}>
                <line x1={loc_x} y1={loc_y} x2={dest_loc_x} y2={dest_loc_y} className={"shadowdash"} />
                <line
                    x1={loc_x}
                    y1={loc_y}
                    x2={dest_loc_x}
