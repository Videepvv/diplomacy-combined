...
class DefaultValueType(ParserType):
    ...
    def update(self, element):
        ...
        return (
            None
            if self.default_json_value is None
            else self.element_type.to_type(self.default_json_value)
        )
    ...
    def to_json(self, raw_value):
        return (
            copy(self.default_json_value)
            if raw_value is None
            else self.element_type.to_json(raw_value)
        )
...
class SequenceType(ParserType):
    ...
    def __init__(self, element_type, sequence_builder=None):
        ...
        self.sequence_builder = (
            sequence_builder if sequence_builder is not None else lambda seq: seq
        )
...
class DictType(ParserType):
    ...
    def __init__(self, key_type, val_type, dict_builder=None):
        ...
        self.key_type = (
            key_type if isinstance(key_type, StringableType) else StringableType(key_type)
        )
        self.val_type = get_type(val_type)
        self.dict_builder = (
            dict_builder if dict_builder is not None else lambda dictionary: dictionary
        )
    ...
    def update(self, element):
        return_dict = {
            self.key_type.update(key): self.val_type.update(value) for key, value in element.items()
        }
        return self.dict_builder(return_dict)

    def to_type(self, json_value):
        json_dict = {
            self.key_type.to_type(key): self.val_type.to_type(value)
    ...
    def to_json(self, raw_value):
        return {
            self.key_type.to_json(key): self.val_type.to_json(value)
...
class IndexedSequenceType(ParserType):
    ...
    def to_type(self, json_value):
        ...
        return self.dict_type.update(
            {getattr(element, self.key_name): element for element in loaded_sequence}
        )
...
class SequenceOfPrimitivesType(ParserType):
    ...
    def __init__(self, seq_of_primitives):
        ...
        """
        assert seq_of_primitives and all(
            primitive in self.primitives for primitive in seq_of_primitives
        )
        self.seq_of_primitives = (
            seq_of_primitives if isinstance(seq_of_primitives, tuple) else tuple(seq_of_primitives)
        )
