...
class ExpectedData:
    ...
    def __init__(self, power_name, phases, phase_index):
        ...
        self.messages = ExpectedMessages(
            power_name, phases[phase_index].get_power_related_messages(power_name)
        )
        self.phases = phases
    ...
    def move_forward(self):
        ...
        if self.__phase_index < len(self.phases):
            self.messages = ExpectedMessages(
                self.messages.power_name,
                self.phases[self.__phase_index].get_power_related_messages(self.power_name),
            )
...
def send_current_orders(game):
    ...
    if not orders_to_send and not game.get_orderable_locations(expected_data.power_name):
        print(
            "%s/no need to send empty orders for unorderable power at phase %s"
            % (expected_data.power_name, expected_data.expected_phase.name)
        )
        return

    print(
        "%s/sending %d orders for phase %s: %s"
        % (
            expected_data.power_name,
            len(orders_to_send),
            expected_data.expected_phase.name,
            orders_to_send,
        )
    )
...
def on_message_received(game, notification):
    ...
    for index, expected_message in enumerate(messages.messages):
        ...
        if expected_message.sender == power_from:
            if notification.message.is_global():
                if not (
                    expected_message.recipient == GLOBAL
                ...
                if not (
                    expected_message.recipient == notification.message.recipient
                    and expected_message.message == notification.message.message
                ):
                    raise AssertionError(
    ...
    if index_found is None:
        raise AssertionError(
            "%s/Received unknown message from %s to %s: %s"
            % (
                power_name,
...
def on_admin_game_phase_update(admin_game, notification=None):
    ...
    print("=" * 80)
    print(
        "We changed phase for admin game, moving from phase",
        expected_data.phase_index,
        "to phase",
        (expected_data.phase_index + 1),
        "/",
        len(expected_data.phases),
    )
    print("=" * 80)

    # state_history must not be empty.
    assert len(admin_game.state_history) == expected_data.phase_index, (
        len(admin_game.state_history),
        expected_data.phase_index,
    )
    ...
    if admin_game.state_history:
        ...
        for power_name in given_orders:
            ...
            assert (
                expected_power_orders == given_power_orders
            ), "Power orders for %s\nExpected: %s\nGiven: %s\nAll given: %s\n" % (
                power_name,
                expected_power_orders,
                given_power_orders,
                given_orders,
            )
        ...
        assert expected_engine.get_hash() == given_engine.get_hash(), (
            expected_engine.get_hash(),
            given_engine.get_hash(),
        )
...
def on_admin_powers_controllers(admin_game, notification):
    ...
    LOGGER.warning(
        ...
        len(
            [
                controller
                for controller in notification.powers.values()
                if controller == strings.DUMMY
            ]
        ),
    )
...
def verify_current_phase(game):
    ...
    print("=" * 80)
    print(
        "Checking expected phase",
        expected_data.expected_phase.name,
        "(%d/%d) for" % (expected_data.phase_index + 1, len(expected_data.phases)),
        expected_data.power_name,
        "with",
        len(expected_data.messages.messages),
        "messages.",
    )
    print("=" * 80)
    # Verify phase name.
...
def load_power_game(case_data, power_name):
    ...
    print(
        "Game created for user %s." % username,
        len(user_game.messages),
        len(user_game.state_history),
    )
...
def main(case_data):
    ...
    if case_data.admin_channel is None:
        ...
        case_data.admin_game.data = ExpectedData(
            power_name="", phases=case_data.phases, phase_index=0
        )
